...
pub fn run_udev(log: Logger) {
    ...
    event_loop
        .handle()
        .insert_source(timer, |(dev_id, crtc), _, consolation_state| {
            consolation_state.render(dev_id, Some(crtc))
        })
        .unwrap();
    ...
    let libinput_event_source = event_loop
        .handle()
        .insert_source(libinput_backend, move |event, _, consolation_state| {
            consolation_state.process_input_event(event)
        })
        .unwrap();
    let session_event_source = event_loop
        .handle()
        .insert_source(notifier, |(), &mut (), _consolation_state| {})
        .unwrap();
    ...
    #[cfg(feature = "egl")]
    {
        ...
        init_dmabuf_global(
            ...
            formats,
            |buffer, mut ddata| {
                let consolation_state = ddata.get::<ConsolationState<UdevData>>().unwrap();
                for backend_data in consolation_state.backend_data.backends.values() {
                    if backend_data
                        .renderer
                        .borrow_mut()
...
struct BackendData {
    ...
    event_dispatcher: Dispatcher<'static, DrmDevice<SessionFd>, ConsolationState<UdevData>>,
    dev_id: u64,
}
    ...
    fn device_added(&mut self, device_id: dev_t, path: PathBuf) {
            ...
            })
        {
            ...
            let restart_token = self
                ...
                .register(move |signal| match signal {
                    SessionSignal::ActivateSession | SessionSignal::ActivateDevice { .. } => {
                        handle.insert_idle(move |consolation_state| {
                            consolation_state.render(dev_id, None)
                        });
                    }
                    _ => {}
                });
            ...
            let event_dispatcher = Dispatcher::new(
                device,
                move |event, _, consolation_state: &mut ConsolationState<_>| match event {
                    DrmEvent::VBlank(crtc) => consolation_state.render(dev_id, Some(crtc)),
                    DrmEvent::Error(error) => {
                        error!(consolation_state.log, "{:?}", error);
                    }
                },
            );
    ...
    fn render(&mut self, dev_id: u64, crtc: Option<crtc::Handle>) {
        ...
        for (&crtc, surface) in to_render_iter {
            ...
            let result = render_surface(
                ...
                &device_backend.font_texture,
                &device_backend.menu_selected_texture,
