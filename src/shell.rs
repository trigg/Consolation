...
use smithay::{
    ...
    reexports::{
        ...
        wayland_server::{
            protocol::{
                wl_buffer, wl_output, wl_pointer::ButtonState, wl_shell_surface, wl_surface,
            },
            Display,
        },
    },
...
pub fn init_shell<BackendData: 'static>(
    ...
    compositor_init(
        ...
        move |surface, mut ddata| {
            let consolation_state = ddata.get::<ConsolationState<BackendData>>().unwrap();
            let window_map = consolation_state.window_map.as_ref();
            let output_map = consolation_state.output_map.as_ref();
    ...
    let (xdg_shell_state, _) = xdg_shell_init(
        ...
        move |shell_event, mut ddata| {
            let state = ddata.get::<ConsolationState<BackendData>>().unwrap();

            match shell_event {
                XdgRequest::NewToplevel { surface } => {
                    // Direct copy from Xdg fullscreen. All toplevel window are fullscreen
                    ...
                    if let Some(current_state) = surface.current_state() {
                            ...
                            .contains(xdg_toplevel::State::Maximized)
                        {
                            ...
                            if fs_changed.is_ok() {
                                ...
                                //
                                // but for consolation needs setting location to pointer location is fine
        ...
        move |req: ShellRequest, mut ddata| {
            ...
            match req {
                ShellRequest::SetKind {
                    ...
                    let max_x = output_geometry.loc.x
                        + (((output_geometry.size.w as f32) / 3.0) * 2.0) as i32;
                    let max_y = output_geometry.loc.y
                        + (((output_geometry.size.h as f32) / 3.0) * 2.0) as i32;
    ...
    smithay::wayland::shell::wlr_layer::wlr_layer_shell_init(
        ...
        move |event, mut ddata| match event {
            LayerShellRequest::NewLayerSurface {
                ...
                let consolation_state = ddata.get::<ConsolationState<BackendData>>().unwrap();
                let output_map = consolation_state.output_map.borrow();

                let output = output.and_then(|output| output_map.find_by_output(&output));
                let output = output.unwrap_or_else(|| {
                    output_map
                        .find_by_position(consolation_state.pointer_location.to_i32_round())
