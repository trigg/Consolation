...
impl<BackendData: Backend> AnvilState<BackendData> {
    ...
    pub fn current_window(&self) -> (Option<Window>, Vec<Window>) {
        // We want the first wayland or X window(excluding override redirect)

        let mut popups = vec![];
        for element in self.elements.clone() {
            if element.is_wayland() {
                return (Some(element.clone()), vec![]);
            }
            match element.x11_surface() {
                Some(x11surface) => {
                    if x11surface.is_override_redirect() {
                        ...
                        return (Some(element.clone()), popups);
                    }
                }
                None => {}
            }
            ...
            (Some(window), xpopups) => {
                ...
                if let Some(surface) = window.wl_surface() {
                    for (popup, location) in PopupManager::popups_for_surface(&surface) {
                        let mut geo = popup.geometry();
                        geo.loc = location - geo.loc;
                        if geo.contains(pointer_location.to_i32_round()) {
                            return Some((popup.into(), geo.loc.to_f64()));
                        }
                    }
                }
                for popup in xpopups {
                    let bbox = popup.bbox();
                    if popup.wl_surface().is_none() {
                        continue;
                    }
                    if bbox.contains(pointer_location.to_i32_round()) {
                        return Some((popup.into(), bbox.loc.to_f64()));
                    }
                }
                Some((window.into(), Point::from((0f64, 0f64))))
            }
            (None, _) => None,
        }
    }
...
impl AnvilState<UdevData> {
    ...
    fn on_pointer_move<B: InputBackend>(
        ...
        if let (Some(window), _xpopups) = self.current_window() {
            // Pass relative movement if possible
            if let Some(surface) = window.wl_surface() {
            ...
            pointer.relative_motion(
                self,
                self.get_pointer_focus(pointer_location),
                &RelativeMotionEvent {
                    delta: evt.delta(),
    ...
    fn on_touch_down<B: InputBackend>(&mut self, evt: B::TouchDownEvent) {
        ...
        let touch_surface = self.get_pointer_focus(touch_location);
        handle.down(
            self,
            touch_surface,
            &DownEvent {
