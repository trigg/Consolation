...
#[derive(Debug)]
pub struct ConsolationState<BackendData> {
    pub backend_data: BackendData,
    pub socket_name: Option<String>,
    pub running: Arc<AtomicBool>,
    pub display: Rc<RefCell<Display>>,
    pub handle: LoopHandle<'static, ConsolationState<BackendData>>,
    ...
    #[cfg(feature = "xwayland")]
    pub xwayland: XWayland<ConsolationState<BackendData>>,
    pub menu_open: bool,
    pub menu_index: i32,
}

impl<BackendData: Backend + 'static> ConsolationState<BackendData> {
    pub fn init(
        display: Rc<RefCell<Display>>,
        handle: LoopHandle<'static, ConsolationState<BackendData>>,
        backend_data: BackendData,
        log: slog::Logger,
        listen_on_socket: bool,
    ) -> ConsolationState<BackendData> {
        // init the wayland connection
        handle
            .insert_source(
                Generic::from_fd(display.borrow().get_poll_fd(), Interest::READ, Mode::Level),
                move |_, _, state: &mut ConsolationState<BackendData>| {
